...
//We will be removing this from our texts.
var DEFAULT_TEXT = 'Welcome to Etherpad!\n\nThis pad text is synchronized as you type, so that everyone viewing this page sees the same text. This allows you to collaborate seamlessly on documents!\n\nGet involved with Etherpad at http://etherpad.org\n\n';
...
function download(session, next){
    ...
    rp(session.url).then(function(body){
        ...
        //Check to see if we have modified the text at all.
        session.default_text = checkDefaultText(body);
...
function checkDefaultText(txt){
    return txt === DEFAULT_TEXT;
}

function countChars(txt){
    txt = (txt || '').replace(DEFAULT_TEXT, '');
    return (txt || '').length;
}
